...
const NAV_ITEMS = [
  { label: 'Home',             to: '/wes-lab' },
  { label: 'Response',        to: '/wes-lab/response' },
  { label: 'Summary',         to: '/wes-lab/summary' },
];

export default function WesLayout({ children }) {
    ...
    <div className={styles.root}>
      <header className={styles.header}>
        ...
        <nav className={styles.nav}>
          {NAV_ITEMS.map(item => {
            const active = pathname === item.to || (item.to !== '/wes-lab' && pathname.startsWith(item.to));
            return (
        ...
        <div className={styles.headerRight}>
          <span className={styles.dashTitle}>WES Lab Assessment</span>
        </div>
      </header>
