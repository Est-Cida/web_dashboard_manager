...
const NAV_ITEMS = [
  { label: 'Home',     to: '/wes-subnational' },
  { label: 'Response', to: '/wes-subnational/response' },
  { label: 'Summary',  to: '/wes-subnational/summary' },
];

export default function WesSubNationalLayout({ children }) {
    ...
    <div className={styles.root}>
      <header className={styles.header}>
        ...
        <nav className={styles.nav}>
          {NAV_ITEMS.map(item => {
            const active = pathname === item.to || (item.to !== '/wes-subnational' && pathname.startsWith(item.to));
            return (
        ...
        <div className={styles.headerRight}>
          <span className={styles.dashTitle}>WES Sub-National Assessment</span>
        </div>
      </header>
